...
A beginner-to-intermediate quantum computing project that builds physical intuition for **superposition, measurement basis, quantum phase, and interference** by comparing a qubit to a classical coin — then breaking that analogy apart using real experiments run on **real IBM Quantum hardware** (Qiskit).
...
- What quantum **phase** is, and how it drives constructive/destructive **interference**.
...
Unlike the classical coin, the quantum coin's coefficients are **complex probability amplitudes**, not probabilities themselves — probability comes from the squared magnitude of each amplitude.
...
Instead, measuring $H|0\rangle$ along X gives an **expectation value of +1** — i.e., a 100% probability of measuring $|+\rangle$ and 0% probability of $|-\rangle$. This is the first clear break from the classical coin analogy: **a state can be random in one measurement basis and completely deterministic in another.**
...
Phase determines how amplitudes interfere — constructively (in phase) or destructively (out of phase). This is demonstrated by applying a Hadamard gate **twice**, which classically would be meaningless, but quantum mechanically uses interference to deterministically return the qubit to its starting state:
...
This flips the interference pattern entirely — the qubit now reliably collapses to $|1\rangle$ instead of $|0\rangle$, proving that a hidden phase (invisible in a single measurement) can completely change the outcome of later operations:
...
A classical NOT gate flips a coin 180°. The **√NOT (square-root-of-NOT) gate** is its quantum analogue at only a 90° rotation — conceptually like a coin balanced on its edge: neither heads nor tails, and collapsing unpredictably to either side upon measurement.

![Circuit applying the √NOT (SX) gate](images/sqrt_not_circuit.png)

Measuring the resulting state along X, Y, and Z using the `Estimator` primitive on real hardware gives expectation values of **0, −1, 0** respectively — 50/50 randomness along X and Z, but a fully deterministic result along Y. This "coin on its edge" is a more accurate physical analogy for superposition than a coin simply flipping in the air, since it reproduces the same "random in one basis, definite in another" behavior seen with the Hadamard gate.

## Part 6: Visualizing Gates on the Bloch Sphere

Every single-qubit gate used in this project can be understood as a deterministic rotation of a vector on the **Bloch sphere**:
...
These visualizations confirm that none of the gate operations are random — the randomness in this entire project comes **only from the act of measurement**, never from the gate operations themselves.
...
- Measuring a quantum superposition state is **not** equivalent to flipping a classical coin — the two initially look identical, but diverge as soon as the measurement basis is changed.
...
- All quantum gate operations are **deterministic and reversible** rotations of this vector; randomness enters the system **only at measurement**.
...
- Builds concepts progressively — coin analogy → basis dependence → phase → interference → Bloch sphere — rather than presenting them in isolation.
...
- Self-contained: environment setup, circuits, execution, and analysis are all in a single reproducible notebook.
...
- The coin analogy, while pedagogically useful, is explicitly acknowledged as breaking down for describing the qubit's state *before* measurement — it is a tool for building early intuition, not a complete physical model.
...
**Muhammad Danyal**
Physics graduate exploring quantum computing, with a focus on building rigorous, hardware-verified intuition for core quantum concepts as part of ongoing preparation for graduate study in quantum computing/physics.
